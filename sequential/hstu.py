...
def _hstu_attention_maybe_from_cache(
    ...
    if delta_x_offsets is not None:
        ...
        padded_k = jagged_to_padded_dense(
    ...
    attn_output = dense_to_jagged(
        torch.einsum(
            "bhnm,bmhd->bnhd",
            qk_attn,
            jagged_to_padded_dense(v, [x_offsets], [n]).reshape(
    ...
    def forward(  # pyre-ignore [3]
        ...
        if self._normalization == "rel_bias" or self._normalization == "hstu_rel_bias":
            ...
            attn_output = dense_to_jagged(
                torch.bmm(
                    qk_attn,
                    jagged_to_padded_dense(v, [x_offsets], [n]),
                ),
                [x_offsets],
            )[0]
        else:
...
class HSTUJagged(torch.nn.Module):
    ...
    def forward(
        ...
        """
        if len(x.size()) == 3:
            x = dense_to_jagged(x, [x_offsets])[0]
...
class HSTU(torch.nn.Module):
    ...
    def generate_user_embeddings(
        ...
        user_embeddings, cached_states = self._hstu(
            x=user_embeddings,
            x_offsets=asynchronous_complete_cumsum(past_lengths),
            # all_timestamps=(
