...
from user_feat.aggregated_features_baseline.constants import (
...
from user_feat.aggregated_features_baseline.features_aggregator import (
    FeaturesAggregator,
)
...
def save_embeddings(
    ...
    """
    logger.info("Saving user features")
    embeddings_dir.mkdir(parents=True, exist_ok=True)
    user_feat = pd.DataFrame({
        "client_id": client_ids,
        "feature": list(embeddings)
    })
    user_feat.to_parquet(embeddings_dir / "user_feat.parquet", index=False)
...
def create_embeddings(
    ...
    # embeddings = np.log1p(embeddings).astype(EMBEDDINGS_DTYPE)
